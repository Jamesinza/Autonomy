...
class DynamicPlasticDenseAdvancedHyper(tf.keras.layers.Layer):
    ...
    def plasticity_update(self, pre_activity, post_activity, reward):
        ...
        features = tf.stack([pre_tile, post_tile, w_val, random_delta], axis=-1)
        features_flat = tf.reshape(features, [-1, 4])
        hyper_params_flat = self.hypernetwork(features_flat)
        hyper_params = tf.reshape(hyper_params_flat, tf.shape(self.w) + (2,))
...
# -----------------------------------------------------------------------------
# 6. Training loop with hypernetwork and architecture modification integration.
# -----------------------------------------------------------------------------
def train_model(model, ds_train, ds_val, ds_test, num_epochs=1000,
                homeostasis_interval=100, architecture_update_interval=100,
                plasticity_start_epoch=20, plasticity_update_interval=10):
    ...
    model.save("models/MetaSynapse_v6e.keras")
...
# =============================================================================
# 3. PlasticDense layer using meta-plasticity.
# =============================================================================
class PlasticDenseAdvancedHyper(tf.keras.layers.Layer):
    ...
    # -----------------------------------------------------------------------------
    # Meta-plasticity update: use hypernetwork outputs for additive and multiplicative factors.
    # -----------------------------------------------------------------------------
    def plasticity_update(self, pre_activity, post_activity, reward):
        in_features = tf.shape(self.w)[0]
        out_features = tf.shape(self.w)[1]
        
        # Prepare features: [pre, post, weight, random_delta]
        pre_broadcast = tf.reshape(pre_activity, (-1, 1))
        pre_tile = tf.tile(pre_broadcast, [1, out_features])
        post_broadcast = tf.reshape(post_activity, (1, -1))
        post_tile = tf.tile(post_broadcast, [in_features, 1])
        w_val = self.w
        random_delta = tf.random.uniform(tf.shape(self.w), minval=-20.0, maxval=20.0)
        ...
        # Hypernetwork outputs two values per connection.
        hyper_params_flat = self.hypernetwork(features_flat)  # shape: (in_features*out_features, 2)
        hyper_params = tf.reshape(hyper_params_flat, tf.shape(self.w) + (2,))
        delta_add = hyper_params[..., 0]
        delta_mult = hyper_params[..., 1]
        
        # Combine additive and multiplicative updates.
        plasticity_delta = tf.cast(reward, tf.float32) * (delta_add + delta_mult * self.w)
        return plasticity_delta
...
def train_model(model, ds_train, ds_val, ds_test, num_epochs=100, homeostasis_interval=100,
    ...
    # Define min, max, and optimal target values for mean and std of plasticity delta
    MEAN_MIN, MEAN_MAX = 1e-4, 1e-3
    STD_MIN, STD_MAX = 5e-4, 1e-2
    OPTIMAL_MEAN = (MEAN_MIN + MEAN_MAX) / 2
    OPTIMAL_STD = (STD_MIN + STD_MAX) / 2
    ...
    # Dynamic adjustment speed settings
    ADJUSTMENT_SPEED = 0.2  # Initial speed
    ADJUSTMENT_SPEED_MIN, ADJUSTMENT_SPEED_MAX = 0.05, 0.5
    rolling_variance = collections.deque(maxlen=10)  # Store last 10 updates
    ...
    global_step = 0
    for epoch in range(num_epochs):
        plasticity_weight = ((epoch - plasticity_start_epoch + 1) / (num_epochs - plasticity_start_epoch + 1)) if epoch >= plasticity_start_epoch else 0.0
        plasticity_weight *= global_plasticity_weight_multiplier
        ...
        for images, labels in ds_train:
            ...
            pre_activity = tf.reduce_mean(x_flat, axis=0)
            post_activity = tf.reduce_mean(hidden, axis=0)
            reward = global_reward_scaling_factor * tf.sigmoid(-loss_val)
            ...
            if plasticity_weight > 0 and global_step % plasticity_update_interval == 0:
                ...
                # Store recent plasticity updates for dynamic adjustment speed
                rolling_variance.append(tf.abs(mean_error) + tf.abs(std_error))
                if len(rolling_variance) == rolling_variance.maxlen:
                    variance_level = (tf.math.reduce_std([rolling_variance[i].numpy() for i in range(len(rolling_variance))])).numpy()
                    
                    if variance_level < 0.1:  # Low variance, increase speed
                        ADJUSTMENT_SPEED = min(ADJUSTMENT_SPEED * 1.1, ADJUSTMENT_SPEED_MAX)
                    elif variance_level > 0.5:  # High variance, decrease speed
                        ADJUSTMENT_SPEED = max(ADJUSTMENT_SPEED * 0.9, ADJUSTMENT_SPEED_MIN)
    ...
    model.save("models/MetaSynapse_v7a.keras")
...
def main():
    ...
    train_model(model, ds_train, ds_val, ds_test, num_epochs=1000, homeostasis_interval=100)
...
def train_model(model, ds_train, ds_val, ds_test, num_epochs=100, homeostasis_interval=100,
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in ds_train:
            ...
            if plasticity_weight > 0 and global_step % plasticity_update_interval == 0:
                ...
                # print(f"  Adjusted Plasticity Multiplier: {global_plasticity_weight_multiplier:.4f}, "
                #       f"Reward Scaling: {global_reward_scaling_factor:.4f}, Adjustment Speed: {ADJUSTMENT_SPEED:.3f}")
    ...
    model.save("models/MetaSynapse_v6f.keras")
...
def train_model(model, ds_train, ds_val, ds_test, num_epochs=100, homeostasis_interval=100,
    ...
    for epoch in range(num_epochs):
        ...
        # Validation & Test Evaluation (unchanged)
        val_loss_metric = tf.keras.metrics.Mean(name='val_loss')
        val_accuracy_metric = tf.keras.metrics.SparseCategoricalAccuracy(name='val_accuracy')
        for images, labels in ds_val:
            predictions, _ = model(images, training=False)
