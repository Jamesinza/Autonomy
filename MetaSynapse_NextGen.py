...
class PlasticityModelMoE(tf.keras.Model):
    def __init__(self, h, w, plasticity_controller, units=128, num_experts=2, max_units=128, 
        ...
        self.cnn1 = tf.keras.layers.Conv2D(16, (3,3), activation='relu', padding='same')
        self.cnn2 = tf.keras.layers.Conv2D(32, (3,3), activation='relu', padding='same')
        self.cnn3 = tf.keras.layers.Conv2D(64, (3,3), activation='relu', padding='same')
        self.drop = tf.keras.layers.Dropout(0.1)
        self.flatten = tf.keras.layers.GlobalAveragePooling2D()
        self.dens = tf.keras.layers.Dense(64, activation='relu', name='pm_dens1')
...
def main():
    ...
    model.summary()
    model_name = "MetaSynapse_NextGen_v8c"
    
    model.load_weights(f"model_weights/{model_name}/model.weights.h5", skip_mismatch=True)
    ...
    train_model(model, model_name, ds_train, ds_val, ds_test, train_steps, val_steps, test_steps,
                ...
                plasticity_update_interval=3, plasticity_start_epoch=3, learning_rate=learning_rate)
    
if __name__ == '__main__':
    main()# Silencing annoying warnings
import shutup
shutup.please()
