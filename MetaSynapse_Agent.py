...
# ================================================
# Hyperparameter Agent: Actor-Critic Architecture
# ================================================
class HyperparameterAgent(tf.keras.Model):
    def __init__(self, state_dim, action_dim, hidden_units=32, **kwargs):
        """
        state_dim: dimensionality of the input state vector.
        action_dim: number of actions (adjustments for:
            plasticity_interval, homeostasis_interval, architecture_interval,
            learning_rate,
            plasticity_threshold_low, plasticity_threshold_high,
            homeostasis_threshold_low, homeostasis_threshold_high,
            architecture_threshold_low, architecture_threshold_high,
            and wait_period) => action_dim = 11.
        """
        super(HyperparameterAgent, self).__init__(**kwargs)
        self.actor = tf.keras.Sequential([
            tf.keras.layers.Dense(hidden_units, activation='relu', input_shape=(state_dim,)),
            tf.keras.layers.Dense(hidden_units, activation='relu'),
            # Tanh outputs in [-1, 1]; these will be scaled appropriately.
...
@tf.function
def train_agent_step(agent, state, action_taken, target_return, advantage, optimizer_agent):
    with tf.GradientTape() as tape:
        action_pred, value = agent(state)
        std = 0.1  # fixed std; can also be learned.
        # dist = tfd.Normal(loc=action_pred, scale=std)
        dist = tf.compat.v1.distributions.Normal(loc=action_pred, scale=std)
        log_prob = tf.reduce_sum(dist.log_prob(action_taken), axis=-1)
        actor_loss = -tf.reduce_mean(log_prob * advantage)
        critic_loss = tf.reduce_mean(tf.square(target_return - tf.squeeze(value)))
        loss = actor_loss + 0.5 * critic_loss
    grads = tape.gradient(loss, agent.trainable_variables)
    optimizer_agent.apply_gradients(zip(grads, agent.trainable_variables))
    return loss

def get_agent_state(hyperparams, model, current_val_loss, avg_update_norm):
    """
    Constructs a state vector that includes current hyperparameters and internal metrics.
    State vector (dim = 14):
      [plasticity_interval, homeostasis_interval, architecture_interval, learning_rate,
       plasticity_threshold_low, plasticity_threshold_high,
       homeostasis_threshold_low, homeostasis_threshold_high,
       architecture_threshold_low, architecture_threshold_high,
       current_val_loss, avg_update_norm,
       avg_homeo_deviation, avg_arch_change]
    """
    avg_homeo_deviation = compute_avg_homeo_deviation(model).numpy()
    avg_arch_change = compute_avg_arch_change(model).numpy()
    state_vector = np.array([
        hyperparams['plasticity_interval'],
        hyperparams['homeostasis_interval'],
        hyperparams['architecture_interval'],
        hyperparams['learning_rate'],
        hyperparams['plasticity_threshold_low'],
        hyperparams['plasticity_threshold_high'],
        hyperparams['homeostasis_threshold_low'],
        hyperparams['homeostasis_threshold_high'],
        hyperparams['architecture_threshold_low'],
        hyperparams['architecture_threshold_high'],
        current_val_loss,
        avg_update_norm,
        avg_homeo_deviation,
        avg_arch_change
...
# ===================
# ================================================
# Integrated Training Function with Agent
# ================================================
def train_model(model, model_name, ds_train, ds_val, ds_test, train_steps, val_steps, test_steps,
    ...
    # Initial hyperparameter settings (all are now learnable via the agent)
    hyperparams = {
        'plasticity_interval': 10.0,
        'homeostasis_interval': 10.0,
        'architecture_interval': 10.0,
        'learning_rate': 1e-3,
        'plasticity_threshold_low': 0.001,
        'plasticity_threshold_high': 0.01,
        'homeostasis_threshold_low': 0.05,
        'homeostasis_threshold_high': 0.2,
        'architecture_threshold_low': 0.01,
        'architecture_threshold_high': 0.1
    }
    ...
    optimizer_agent = tf.keras.optimizers.Adam(learning_rate=1e-3)
    ...
    # === Adaptive Mechanisms (initial settings) ===
    adaptive_plasticity_interval = 10.0  # initial plasticity update interval
    ...
    adaptive_homeostasis_interval = 10.0  # initial homeostasis interval
    homeo_threshold_low = 0.05
    homeo_threshold_high = 0.2
    min_homeo_interval = 1.0
    max_homeo_interval = 100.0

    adaptive_architecture_interval = 10.0  # initial architecture update interval
    arch_threshold_low = 0.01
    arch_threshold_high = 0.1
    min_arch_interval = 1.0
    max_arch_interval = 100.0

    current_lr = initial_lr
    lr_patience_counter = 0

    # === Autonomous Agent Setup ===
    # State dimension is 14 as defined above; action dimension is 11.
    agent = HyperparameterAgent(state_dim=14, action_dim=11)

    # For agent episodes.
    episode_start_val_loss = None
    agent_episode_data = []  # To store (state, action, reward)
    discount_factor = 0.99

    # Main training loop, integrated with agent episodes.
    global_epoch = 0
    while global_epoch < num_epochs:
        ...
        if global_epoch >= plasticity_start_epoch:
            plasticity_weight_val = ((global_epoch - plasticity_start_epoch + 1) /
        ...
        for step in range(train_steps):
            ...
            (loss_value, recon_loss, predictions, combined_input,
             hidden, latent, gating_signal, alignment_score) = train_step(
                    model, loss_fn, recon_loss_fn, optimizer, ae_optimizer, critic_optimizer,
            ...
            reward, pre_activity, post_activity = compute_reinforcement_signals(
                    model, combined_input, hidden, loss_value, recon_loss, global_reward_scaling_factor)
            ...
            if plasticity_weight_val > 0 and tf.equal(global_step % adaptive_plasticity_interval, 0):
                delta_ws, delta_delays = scheduled_plasticity_updates(
                    model, global_plasticity_weight_multiplier, global_reward_scaling_factor,
                    pre_activity, post_activity, gating_signal, reward, neuromod_signal)
        ...
        # --- End-of-Epoch Adjustments for Update Intervals ---
        ...
        # Epoch logging.
        tf.print(f"\nEpoch {global_epoch+1}/{num_epochs}\n"
        ...
        # --- Agent Update: Build State & Get Agent Action ---
        # Evaluate validation loss over val_steps.
        for _ in range(val_steps):
            images, labels = next(val_iter)
            preds, *_ = model(images, training=False)
            loss_val = loss_fn(labels, preds)
            val_loss_metric.update_state(loss_val)
        current_val_loss = val_loss_metric.result().numpy()
        val_loss_metric.reset_state()

        # Use plasticity update norm average computed above (or 0 if none).
        avg_update_norm_val = avg_update_norm if 'avg_update_norm' in locals() else 0.0
        state = get_agent_state(hyperparams, model, current_val_loss, avg_update_norm_val)
        if episode_start_val_loss is None:
            episode_start_val_loss = current_val_loss

        # Agent selects an action.
        action_raw, _ = agent(state)
        action = action_raw.numpy().squeeze()

        # Scale the adjustments.
        delta_plast = action[0] * 5.0
        delta_homeo = action[1] * 5.0
        delta_arch = action[2] * 5.0
        delta_lr = action[3] * 1e-4
        # Map action[4] from [-1,1] to a wait period in [1, 10] epochs.
        wait_period = int(((action[4] + 1) / 2) * 9) + 1

        delta_plast_thresh_low = action[5] * 1e-3
        delta_plast_thresh_high = action[6] * 1e-3
        delta_homeo_thresh_low = action[7] * 1e-3
        delta_homeo_thresh_high = action[8] * 1e-3
        delta_arch_thresh_low = action[9] * 1e-3
        delta_arch_thresh_high = action[10] * 1e-3

        # Update hyperparameters (with clipping).
        hyperparams['plasticity_interval'] = np.clip(hyperparams['plasticity_interval'] + delta_plast, 1, 500)
        hyperparams['homeostasis_interval'] = np.clip(hyperparams['homeostasis_interval'] + delta_homeo, 1, 500)
        hyperparams['architecture_interval'] = np.clip(hyperparams['architecture_interval'] + delta_arch, 1, 500)
        hyperparams['learning_rate'] = np.clip(hyperparams['learning_rate'] + delta_lr, 1e-6, 1e-2)
        hyperparams['plasticity_threshold_low'] = np.clip(hyperparams['plasticity_threshold_low'] + delta_plast_thresh_low, 1e-5, 1e-1)
        hyperparams['plasticity_threshold_high'] = np.clip(hyperparams['plasticity_threshold_high'] + delta_plast_thresh_high, 1e-5, 1e-1)
        hyperparams['homeostasis_threshold_low'] = np.clip(hyperparams['homeostasis_threshold_low'] + delta_homeo_thresh_low, 1e-5, 1e-1)
        hyperparams['homeostasis_threshold_high'] = np.clip(hyperparams['homeostasis_threshold_high'] + delta_homeo_thresh_high, 1e-5, 1e-1)
        hyperparams['architecture_threshold_low'] = np.clip(hyperparams['architecture_threshold_low'] + delta_arch_thresh_low, 1e-5, 1e-1)
        hyperparams['architecture_threshold_high'] = np.clip(hyperparams['architecture_threshold_high'] + delta_arch_thresh_high, 1e-5, 1e-1)

        tf.print(f"\nAgent update at epoch {global_epoch+1}:")
        tf.print("New hyperparameters:", hyperparams, "Wait period:", wait_period)

        # Update model optimizers with new learning rate.
        optimizer.learning_rate.assign(hyperparams['learning_rate'])
        ae_optimizer.learning_rate.assign(hyperparams['learning_rate'])
        critic_optimizer.learning_rate.assign(hyperparams['learning_rate'])

        # Agent's reommended hyperparams
        adaptive_plasticity_interval = tf.cast(tf.round(hyperparams['plasticity_interval']), dtype=tf.float32)
        adaptive_homeostasis_interval = tf.cast(tf.round(hyperparams['homeostasis_interval']), dtype=tf.float32)
        adaptive_architecture_interval = tf.cast(tf.round(hyperparams['architecture_interval']), dtype=tf.float32)
        plasticity_threshold_low = hyperparams['plasticity_threshold_low']
        plasticity_threshold_high = hyperparams['plasticity_threshold_high']
        homeo_threshold_low = hyperparams['homeostasis_threshold_low']
        homeo_threshold_high = hyperparams['homeostasis_threshold_high']
        arch_threshold_low = hyperparams['architecture_threshold_low']
        arch_threshold_high = hyperparams['architecture_threshold_high']

        # Start Episode Validation
        # Run training for the duration of the agent's wait period
        # to check impact of the recommended params before rewarding agent
        episode_epochs = wait_period
        new_val_loss = np.inf
        tf.print(f'\nTesting the Agents recommended params for {wait_period} epoch/s')
        tf.print(f"\nProposed Plasticity Interval: {adaptive_plasticity_interval}\n"
                 f"Proposed Homeostasis Interval: {adaptive_homeostasis_interval}\n"
                 f"Proposed Architecture Interval: {adaptive_architecture_interval}\n")        
        for ep in range(episode_epochs):
            global_epoch += 1
            train_iter = iter(ds_train)
            for step in range(train_steps):
                images, labels = next(train_iter)
                # Calling the train_step function
                (loss_value, recon_loss, predictions, combined_input,
                 hidden, latent, gating_signal, alignment_score) = train_step(
                        model, loss_fn, recon_loss_fn, optimizer, ae_optimizer, critic_optimizer,
                        images, labels, plasticity_weight_val,
            ...
            # --- Apply Agent's Reccomendations for Update Intervals ---
            if plasticity_update_count > 0:
                avg_update_norm = plasticity_update_norm_sum / plasticity_update_count
                if avg_update_norm < plasticity_threshold_low:
                    adaptive_plasticity_interval = max(min_interval, adaptive_plasticity_interval - 1)
            ...
            val_step(model, loss_fn, recon_loss_fn, val_loss_metric, val_accuracy_metric,
                     val_recon_loss_metric, val_iter, val_steps)
            new_val_loss = val_loss_metric.result().numpy()
            current_val_recon_loss = val_recon_loss_metric.result().numpy()
            tf.print(f"Val Loss   : {new_val_loss:.4f}, "
        ...
        # End-of-episode validation.
        
        # Delayed reward: improvement in validation loss (discounted).
        reward_ep = (episode_start_val_loss - new_val_loss) * (discount_factor ** episode_epochs)
        tf.print(f"\nAgent episode: start_loss={episode_start_val_loss:.4f}, end_loss={new_val_loss:.4f}, reward={reward_ep:.6f}\n")
        agent_episode_data.append((state, action_raw, reward_ep))
        episode_start_val_loss = new_val_loss

        # Update the agent from the collected episode data.
        for (s, a, r) in agent_episode_data:
            target_return = tf.cast(r, tf.float32)  # Ensure target_return is float32.
            _, value_est = agent(s)
            advantage = target_return - tf.squeeze(value_est)
            _ = train_agent_step(agent, s, a, target_return, advantage, optimizer_agent)
        agent_episode_data = []

        tf.print(f"Epoch {global_epoch+1} completed; time taken: {time.time() - start_time:.2f}s\n")

        # --- Adaptive Learning Rate Update ---
        if new_val_loss < best_val_loss - early_stop_min_delta:
    ...
    tf.print(f"\nTraining complete. Final global epoch: {global_epoch+1}")
...
def main():
    ...
    model_name = "MetaSynapse_Agent_v1a"
    train_model(model, model_name, ds_train, ds_val, ds_test, train_steps, val_steps, test_steps,
                num_epochs=10000, plasticity_start_epoch=1, early_stop_patience=1000,
                early_stop_min_delta=1e-4, initial_lr=1e-3, min_lr=1e-6, lr_decay_factor=0.9,
                lr_patience=10, critic_loss_weight=0.1)
