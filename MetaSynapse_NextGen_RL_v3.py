...
class DynamicPlasticDenseDendritic(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.spars_max = self.add_weight(
            shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.5, maxval=0.8),
            trainable=True, name="spars_max")        
        self.spars_min = self.add_weight(
            shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.4),
            trainable=True, name="spars_min")
        self.weight_mag_max = self.add_weight(
            shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.06, maxval=0.1),
            trainable=True, name="weight_mag_max")        
        self.weight_mag_min = self.add_weight(
            shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.01, maxval=0.05),
            trainable=True, name="weight_mag_min")
    ...
    def adjust_prune_threshold(self):
        self.prune_threshold.assign(tf.cond(
            tf.greater(self.sparsity, self.spars_max),
            lambda: self.prune_threshold * self.prune_min,
            lambda: tf.cond(tf.less(self.sparsity, self.spars_min),
    ...
    def adjust_add_prob(self):
        self.add_prob.assign(tf.cond(
            tf.less(self.avg_weight_magnitude, self.weight_mag_min),
            lambda: self.add_prob * self.prob_min,
            lambda: tf.cond(tf.greater(self.avg_weight_magnitude, self.weight_mag_max),
...
class MoE_DynamicPlasticLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.clip1 = self.add_weight(
            shape=(),
            initializer=tf.keras.initializers.RandomUniform(minval=0.01, maxval=0.09),
            trainable=True,
            name='clip1_norm')
        self.clip2 = self.add_weight(
            ...
            trainable=True,
            name='clip2_norm')
        # Trainable interval updates
        self.plasticity_interval = self.add_weight(
            name='plasticity_interval', shape=(), initializer=tf.keras.initializers.Constant(1.0), trainable=True)
        self.homeostasis_interval = self.add_weight(
            name='homeostasis_interval', shape=(), initializer=tf.keras.initializers.Constant(1.0), trainable=True)
        self.architecture_interval = self.add_weight(
            name='architecture_interval', shape=(), initializer=tf.keras.initializers.Constant(1.0), trainable=True)
        # Threshold triggers
        self.plast_gate_trigger = self.add_weight(
            name='plast_gate_trigger', shape=(),
            initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.9), trainable=True)
        self.homeo_gate_trigger = self.add_weight(
            name='homeo_gate_trigger', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.9), trainable=True)
        self.arch_gate_trigger = self.add_weight(
            name='arch_gate_trigger', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.9), trainable=True)
        # Weights for aux losses
        self.lambda_plast_weight = self.add_weight(
            name='lambda_plast_weight', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.0, maxval=1.0), trainable=True)
        self.lambda_homeo_weight = self.add_weight(
            name='lambda_homeo_weight', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.0, maxval=1.0), trainable=True)
        self.lambda_arch_weight = self.add_weight(
            name='lambda_arch_weight', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.0, maxval=1.0), trainable=True)
        self.target_update_norm_weight = self.add_weight(
            name='target_update_norm_weight', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.05, maxval=0.5), trainable=True)
        self.target_homeo_weight = self.add_weight(
            name='target_homeo_weight', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.05, maxval=0.5), trainable=True)
        self.target_arch_weight = self.add_weight(
            name='target_arch_weight', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.01, maxval=0.1), trainable=True)        
...
class PlasticityModelMoE(tf.keras.Model):
    def __init__(self, h, w, units=128, num_experts=1, num_layers=1, max_units=128, 
        ...
        # Other
        self.compute_latent_mod = tf.keras.Sequential([tf.keras.layers.Dense(16, activation='relu', name='latmod_dens1'),
    ...
    def build(self, input_shape):
        ...
        # self.architecture_interval = self.add_weight(
        #     name='architecture_interval', shape=(), initializer=tf.keras.initializers.Constant(1.0), trainable=True)
        # # Trainable threshold adjustments
        # # self.plasticity_threshold_low = self.add_weight(
        # #     name='plasticity_threshold_low', shape=(), initializer=tf.keras.initializers.Constant(0.01), trainable=True)
        # # self.plasticity_threshold_high = self.add_weight(
        # #     name='plasticity_threshold_high', shape=(), initializer=tf.keras.initializers.Constant(0.06), trainable=True)
        # # self.homeostasis_threshold_low = self.add_weight(
        # #     name='homeostasis_threshold_low', shape=(), initializer=tf.keras.initializers.Constant(0.003), trainable=True)
        # # self.homeostasis_threshold_high = self.add_weight(
        # #     name='homeostasis_threshold_high', shape=(), initializer=tf.keras.initializers.Constant(0.3), trainable=True)
        # # self.architecture_threshold_low = self.add_weight(
        # #     name='architecture_threshold_low', shape=(), initializer=tf.keras.initializers.Constant(0.001), trainable=True)
        # # self.architecture_threshold_high = self.add_weight(
        # #     name='architecture_threshold_high', shape=(), initializer=tf.keras.initializers.Constant(0.2), trainable=True)
        # # Threshold triggers
        # self.plast_gate_trigger = self.add_weight(
        #     name='plast_gate_trigger', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.9), trainable=True)
        # self.homeo_gate_trigger = self.add_weight(
        #     name='homeo_gate_trigger', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.9), trainable=True)
        # self.arch_gate_trigger = self.add_weight(
        #     name='arch_gate_trigger', shape=(), initializer=tf.keras.initializers.RandomUniform(minval=0.1, maxval=0.9), trainable=True)
    ...
    def call(self, x, training=False):
        ...
        drop = self.drop(combined2, training=training)

         # --- Main outputs ---        
        class_out = self.classification_head(drop)
...
def val_step(model, loss_fn, recon_loss_fn, val_loss_metric, val_accuracy_metric,
    ...
    for step in range(val_steps):
        ...
        # val_res = val_loss_metric.result().numpy()
        # if step % 1 == 0:
        #     tf.print(f"Training loss at step {step}/{val_steps}: {val_res:.4f}")
...
# @tf.function
def train_step(model, loss_fn, recon_loss_fn, optimizer, images, labels, plasticity_weight_val,
                    train_loss_metric, train_accuracy_metric, train_recon_loss_metric,
                    mean_alignment_metric, mean_gating_metric, plasticity_update_norm_sum,
                    global_plasticity_weight_multiplier, global_reward_scaling_factor, global_step,
                    plasticity_update_count):
    
    with tf.GradientTape(persistent=False) as tape:
        (predictions, combined_input, hidden, reconstruction,
         latent, gating_signal, alignment_score, loss_value, recon_loss) = forward_pass(model, images, labels, loss_fn,
                                                                                        recon_loss_fn, mean_alignment_metric)
        
        # Compute reinforcement and neuromodulatory signals.
        reward, pre_activity, post_activity = compute_reinforcement_signals(model, combined_input, hidden, loss_value,
                                                                            recon_loss, global_reward_scaling_factor)
        base_neuromod_signal = compute_neuromodulatory_signal(predictions, reward)
        latent_signal = tf.reduce_mean(model.compute_latent_mod(latent))
        neuromod_signal = base_neuromod_signal * latent_signal

        plasticity_update_count, plasticity_update_norm_sum = soft_gating_plast(model, global_plasticity_weight_multiplier,
                                                                                global_reward_scaling_factor, pre_activity, global_step,
                                                                                post_activity, gating_signal, reward, neuromod_signal,
                                                                                plasticity_update_norm_sum, plasticity_update_count)
        soft_gating_homeo_arch(model, global_step)

        # Compute average metrics.
        avg_update_norm = (plasticity_update_norm_sum / plasticity_update_count) if plasticity_update_count > 0 else 0.0
        avg_homeo_deviation = compute_avg_homeo_deviation(model)
        avg_arch_change = compute_avg_arch_change(model)
    
        # Define auxiliary losses.
        L_plast = tf.cast(model.hidden.lambda_plast_weight * tf.square(avg_update_norm - model.hidden.target_update_norm_weight), tf.float32)
        L_homeo = tf.cast(model.hidden.lambda_homeo_weight * tf.square(avg_homeo_deviation - model.hidden.target_homeo_weight), tf.float32)
        L_arch  = tf.cast(model.hidden.lambda_arch_weight  * tf.square(avg_arch_change - model.hidden.target_arch_weight), tf.float32)
        L_aux = L_plast + L_homeo + L_arch

        # tf.print(f'\ntarget_update_norm_weight: {model.target_update_norm_weight.numpy()}')
        # tf.print(f'target_homeo_weight: {model.target_homeo_weight.numpy()}')
        # tf.print(f'avg_arch_change: {avg_arch_change}')
        # tf.print(f'L_aux: {L_aux}')

        # tf.print(f'\navg_update_norm: {avg_update_norm}')

        # tf.print(f'\nL_plast: {L_plast}')
        # tf.print(f'L_homeo: {L_homeo}')
        # tf.print(f'L_arch: {L_arch}')

        # tf.print(f'\nloss_value: {loss_value}')
        # tf.print(f'L_aux: {L_aux}')
        # tf.print(f'recon_loss: {recon_loss}')
        # tf.print(f'alignment_score: {alignment_score}')
    
        total_loss = loss_value + L_aux + recon_loss + alignment_score
    ...
    del tape
    return (total_loss, recon_loss, L_aux, predictions, combined_input,
            hidden, latent, gating_signal, alignment_score, avg_update_norm,
            avg_homeo_deviation, avg_arch_change
           )

@tf.function
def forward_pass(model, images, labels, loss_fn, recon_loss_fn, mean_alignment_metric):
    (predictions, combined_input, hidden, reconstruction,
     latent, gating_signal, alignment_score) = model(images, training=True)
    # Compute main losses.
    loss_value = loss_fn(labels, predictions)
    recon_loss = recon_loss_fn(images, reconstruction)  
    critic_loss = mean_alignment_metric(alignment_score)
    return (predictions, combined_input, hidden, reconstruction,
            latent, gating_signal, critic_loss, loss_value, recon_loss)

# @tf.function
def soft_gating_plast(model, global_plasticity_weight_multiplier, global_reward_scaling_factor,
                      pre_activity, global_step, post_activity, gating_signal, reward, neuromod_signal,
                      plasticity_update_norm_sum, plasticity_update_count):
    plast_gate = tf.sigmoid(10.0 - tf.math.floormod(global_step, model.hidden.plasticity_interval))
    if plast_gate > model.hidden.plast_gate_trigger:  # A learnable threshold trigger
        tf.print(f'\nglobal_step: {global_step.numpy()}')
        tf.print(f'plast_gate: {plast_gate}')
        tf.print(f'plast_gate_trigger: {model.hidden.plast_gate_trigger.numpy()}')
        delta_ws, delta_delays = scheduled_plasticity_updates(model, global_plasticity_weight_multiplier,
                                                              ...
                                                              post_activity, gating_signal, reward,
                                                              neuromod_signal)
        total_delta_norm = tf.norm(delta_ws) + tf.norm(delta_delays)
        plasticity_update_norm_sum += total_delta_norm.numpy()
        # plasticity_update_count += 1
        plasticity_update_count.assign_add(1)
        tf.print(f'plasticity_update_norm_sum: {plasticity_update_norm_sum}')
        tf.print(f'plasticity_update_count: {plasticity_update_count.numpy()}')
    return plasticity_update_count, plasticity_update_norm_sum

@tf.function
def soft_gating_homeo_arch(model, global_step):
    # Soft gating for homeostasis and architecture updates:
    homeo_gate = tf.sigmoid(10.0 - tf.math.floormod(global_step, model.hidden.homeostasis_interval))
    arch_gate = tf.sigmoid(10.0 - tf.math.floormod(global_step, model.hidden.architecture_interval))
    if homeo_gate > model.hidden.homeo_gate_trigger: # A learnable threshold trigger
        for expert in model.hidden.experts:
            expert.apply_homeostatic_scaling()
            expert.apply_structural_plasticity()
    if arch_gate > model.hidden.arch_gate_trigger: # A learnable threshold trigger
        for expert in model.hidden.experts:
...
def scheduled_plasticity_updates(model, global_plasticity_weight_multiplier, global_reward_scaling_factor,
    ...
    gpwm = global_plasticity_weight_multiplier.numpy()
    update_chaos(model, gpwm, global_plasticity_weight_multiplier, global_reward_scaling_factor)
    ...
    scale_factor, bias_adjustment, update_gate = run_meta_plasticity_controller(model, plasticity_features)
    ...
    del plasticity_features, all_ws
    return delta_ws, delta_delays
...
@tf.function
def update_chaos(model, gpwm, global_plasticity_weight_multiplier, global_reward_scaling_factor):
    chaotic_factor = model.chaotic_modulator(gpwm)
    global_plasticity_weight_multiplier.assign(chaotic_factor)
    global_reward_scaling_factor.assign(chaotic_factor)

@tf.function
def run_meta_plasticity_controller(model, plasticity_features):
...
def train_model(model, model_name, ds_train, ds_val, ds_test, train_steps, val_steps, test_steps,
                num_epochs=1000, plasticity_start_epoch=1,
                early_stop_patience=1000, early_stop_min_delta=1e-4, initial_lr=1e-3,
    ...
    # Target values and lambda weights for the auxiliary losses.
    # target_update_norm = model.target_update_norm_weight  # Desired average plasticity update norm.
    # target_homeo = model.target_homeo_weight              # Desired average homeostatic deviation.
    # target_arch = model.target_arch_weight                # Desired average architecture change.
    # lambda_plast = model.lambda_plast_weight              # Loss contribution
    # lambda_homeo = model.lambda_homeo_weight
    # lambda_arch = model.lambda_arch_weight
    ...
    # For tracking plasticity update magnitude.
    plasticity_update_norm_sum = 0.0
    # plasticity_update_count = 0
    plasticity_update_count = tf.Variable(0.0, dtype=tf.float32)
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(train_steps):
            ...
            (loss_value, recon_loss, L_aux, predictions, combined_input,
            hidden, latent, gating_signal, alignment_score, avg_update_norm,
            avg_homeo_deviation, avg_arch_change) = train_step(
                    model, loss_fn, recon_loss_fn, optimizer,
                    images, labels, plasticity_weight_val,
                    train_loss_metric, train_accuracy_metric, train_recon_loss_metric,
                    mean_alignment_metric, mean_gating_metric, plasticity_update_norm_sum,
                    global_plasticity_weight_multiplier, global_reward_scaling_factor, global_step,
                    plasticity_update_count)
        ...
        # Reset plasticity tracking.
        plasticity_update_norm_sum = 0.0
        plasticity_update_count.assign(0)

        tf.print(f"\nEpoch {epoch+1} update metrics:")
        tf.print(f"Avg Plasticity Update Norm: {avg_update_norm:.4f}")
        tf.print(f"Avg Homeostatic Deviation: {avg_homeo_deviation:.4f}")
        tf.print(f"Avg Architecture Change: {avg_arch_change:.4f}")
        # tf.print(f"Auxiliary Loss: {L_aux:.6f}")
        tf.print(f"Learned Plasticity Interval: {model.hidden.plasticity_interval.numpy():.2f}")
        tf.print(f"Learned Homeostasis Interval: {model.hidden.homeostasis_interval.numpy():.2f}")
        tf.print(f"Learned Architecture Interval: {model.hidden.architecture_interval.numpy():.2f}")
        ...
        if current_val_loss < best_val_loss - early_stop_min_delta:
            ...
            if lr_patience_counter >= lr_patience:
                ...
                if new_lr < current_lr:
                    ...
                    tf.print(f"\nReduced learning rate to {current_lr:.6e}")
                lr_patience_counter = 0
        ...
        if current_val_loss < best_val_loss - early_stop_min_delta:
            ...
            if patience_counter >= early_stop_patience:
                if best_weights is not None:
                    tf.print("Restoring best weights and saving final model.")
                    model.set_weights(best_weights)
                    model.save(f"models/{model_name}.keras")
                break

        tf.print(f"\nTime taken for epoch {epoch+1}: {time.time() - start_time:.2f}s\n")
...
def main():
    ...
    _ = model.critic(tf.zeros([1,16]),tf.zeros([1,10]),tf.zeros([1,64]),training=False)  # memory, preds, hidden
    ...
    _ = model.hidden.experts[0](tf.random.normal((1, 320)), training=False)
    model.summary()
    # model.load_weights("model_weights/MetaSynapse_NextGen_RL_v2/model.weights.h5", skip_mismatch=True)
    model_name = "MetaSynapse_NextGen_RL_v3a"
    train_model(model, model_name, ds_train, ds_val, ds_test, train_steps, val_steps, test_steps,
                num_epochs=1, plasticity_start_epoch=1,
                early_stop_patience=1000, early_stop_min_delta=1e-4, initial_lr=learning_rate,
                min_lr=1e-6, lr_decay_factor=0.9, lr_patience=2, critic_loss_weight=0.1)
